...
export default function SearchModel({ setSearchModel }) {
  ...
  return (
    <motion.section
      ...
      className="fixed bg-navLinks z-20 top-0 left-0 right-0 h-[75%] md:p-6"
    >
        ...
        <div className="mt-36 md:p-12">
          <div className="relative" id="input">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && filterProducts.length > 0) {
                  setSearchModel(false);
                  router.push(`/shop/${filterProducts[0].slug}`);
                }
              }}
            />
            <div className="mt-10 flex flex-col gap-4 h-32 overflow-auto">
              {searchTerm && filterProducts.length > 0 ? (
                filterProducts.map((product) => (
                  <div
                    ...
                    className="bg-gray-100 hover:bg-zinc-800 hover:text-main text-black p-2 shadow-md hover:shadow-lg transition cursor-pointer flex justify-between items-center"
                  >
